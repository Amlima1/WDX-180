...
// 4) Extra
const users02 = "https://jsonplaceholder.typicode.com";

async function asyncInit02(){
    try{
        let result = await fetch(users02);

        result = await result.json();

        console.log( "3) Result using async/await:", result[0].name, result[0].email );
    } catch(err){
